import sys
import numpy as np  # Pre-import to stabilize binary loading
...
MACHINE_DISPLAY = {
    ...
    'CNC_Milling':         {'label': 'CNC Milling C',   'power': 11.0, 'sensor': 'IFM VVB001',       'cost': '₹20,000'},
    'Induction_Hardener':  {'label': 'Induction Har',   'power': 18.0, 'sensor': 'Schneider PM5000', 'cost': '₹15,000'},
    'Hydraulic_Press':     {'label': 'Hydraulic Spl',   'power': 22.0, 'sensor': 'Schneider PM5000', 'cost': '₹15,000'},
    'Cylindrical_Grinder': {'label': 'Cylindrical G',   'power':  7.5, 'sensor': 'IFM VVB001',       'cost': '₹20,000'},
}
...
class DriveShaftProductionEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.completed >= self.n_jobs:
            ...
            reward -= 200.0 * (self.n_jobs - self.completed)
            done    = True
...
def run_ai(model, health_scenario):
    ...
    obs = env._get_obs() # Resync health
    ...
    while not done:
        ...
        log.append({
            ...
            'health':    env.health.get(action, 1.0) if 1 <= action <= 5 else 1.0,
            'completed': info['completed'],
        })
        step += 1
    return log, info

def run_fifo(health_scenario):
    env = DriveShaftProductionEnv(n_jobs=10)
    obs, _ = env.reset()
    env.health = health_scenario.copy()
    log, done, step = [], False, 0
    while not done:
        ...
        for j in range(env.n_jobs):
            if env.job_stages[j] < 5:
                action = env.job_stages[j] + 1
                break
        obs, _, done, _, info = env.step(action)
        log.append({
            'action':    action,
...
def cost_inr(log):
    return round(sum(
        MACHINE_POWER_KW[e['action']] * e['tariff'] * SLOT_HOURS * e['health']
...
if run_btn:
    if model is None:
        ...
        with st.spinner('Running simulation...'):
            ...
            fi_log, fi_info = run_fifo(HEALTH_SCENARIO)
        ...
        st.session_state.update({
            'sim_done': True, 'sim_error': False,
            'ai_log': ai_log, 'fi_log': fi_log,
            'ai_info': ai_info, 'fi_info': fi_info,
            'ai_c': ai_c, 'fi_c': fi_c, 'sav': sav,
            'pct': pct, 'co2': co2, 'press_health': press_health,
        })
...
with tab1:
    ...
    st.markdown('**Component:** Rear axle half-shaft for B-segment passenger vehicles (Maruti Swift, Honda Amaze). 40 units per 8-hour shift.')
    ...
    for mn, md in MACHINE_DISPLAY.items():
        ...
        spec_rows.append({'Machine': md['label'], 'Rated Power': f"{md['power']}kW", 'Sensor': md['sensor'], 'Sensor Cost': md['cost'], 'Annual Status': f"{icon} {iso}"})
...
with tab2:
    ...
    if sim_error:
        ...
        ai_c, fi_c, sav, pct, co2 = st.session_state['ai_c'], st.session_state['fi_c'], st.session_state['sav'], st.session_state['pct'], st.session_state['co2']
...
with tab3:
    ...
    if sim_done:
        ...
        MNAME_FULL = {0:'WAIT', 1:'CNC Turning', 2:'CNC Milling', 3:'Induction Hardener', 4:'Hydraulic Press', 5:'Cylindrical Grinder'}
        for i, e in enumerate(ai_log[-15:]):
            a, t, h = e['action'], e['tariff'], e.get('health', 1.0)
            step_num = len(ai_log) - 15 + i + 1
            reason = f'Tariff ₹{t:.0f}/kWh — deferring' if a == 0 else (f'Press used despite {h:.2f}x penalty' if a == 4 and h > 1.08 else f'Optimal routing | Tariff ₹{t:.0f}')
...
with tab4:
    ...
    if sim_done:
        ai_c, fi_c, sav, pct, ph = st.session_state['ai_c'], st.session_state['fi_c'], st.session_state['sav'], st.session_state['pct'], st.session_state['press_health']
        ...
        bars = ax.bar(['FIFO\n(Manual)', 'PPO Agent\n(IEIS)'], [fi_c, ai_c], color=['#EF5350', '#42A5F5'], edgecolor='black', width=0.45)
        ax.set_ylabel('Shift Energy Cost (₹)')
        ax.set_title(f'Cost Comparison — Press Health {ph:.2f}x')
...
- Shift saving: **₹{max(sav,0):.0f}** ({pct}%)
- Projected annual saving: **₹{annual:,.0f}**
- Payback period: **{payback} shifts**
- Carbon offset: **{round((max(sav,0)/8)*0.82,1)} kg CO₂**
